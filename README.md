...
| - | - |
| начальные и конечные точки маршрута для разных судов | start_coords: coords; end_coords: coords |
| Пиковая скорость по чистой воде |	max_vel: int |
| Ледовый класс |	arc: str |
| Наименование судна |	name: str |
| Дата начала плавания |	date: str |
...
|- | - |
| Наименование судна | name: str | 
| Время отправления | date: str |
| Время прибытия | date: str |
| Диаграмма ганта | data: binary |


<!-- 
Мы получаем запрос от обычного судна желающего проследовать по северному морскому пути в сопровождении ледокола, чтобы не разбить об лед. Система обрабатывает запрос и на выходе обновляет состояние в базе данных отвечающее за результат.
...
У нас есть сервис для обработки документов построенный на этих моделях:
```
...
Перепиши код и модели для обработки документов под нашу задачу
 -->